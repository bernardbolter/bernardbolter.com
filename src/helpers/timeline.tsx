...
// Extended artwork interface with all timeline data
export interface TimelineArtwork extends Artwork {
  ...
  isMissingYear: boolean; // Flag to indicate if this is a missing year entry
  originalIndex?: number; // Only present for real artworks
}
...
// Timeline result with all calculated data
export interface TimelineResult {
  artworkScrollArray: TimelineArtwork[]; // Only real artworks for scrolling
  timelineDisplayArray: TimelineArtwork[]; // Artworks + missing years for timeline display
...
// Helper function to create missing year entry
function createMissingYearEntry(year: number): TimelineArtwork {
  ...
  return {
    ...
    isMissingYear: true,
    // Add any other required Artwork properties with default values
    artist: '',
    description: '',
    imageUrl: '',
    category: '',
...
/**
 * Main function to generate complete timeline data
 */
...
export function generateTimeline(config: TimelineConfig): TimelineResult {
  ...
  // Step 3: Generate timeline entries with missing years (only for latest/oldest)
  let timelineDisplayArray: TimelineArtwork[] = [];
  let artworkScrollArray: TimelineArtwork[] = [];
  
  if (sorting === 'random') {
    // For random: no margins, no missing years
    const randomArtworks = sortedArtworks.map(artwork => ({
      ...artwork,
      timeDifference: 0,
      timeMargin: 0,
      yearsDifference: 0,
    ...
    artworkScrollArray = [...randomArtworks];
    timelineDisplayArray = [...randomArtworks]; // Same for random
  } else {
    // For latest/oldest: calculate margins and add missing years
    const allYears = new Set<number>();
    const artworksByYear = new Map<number, (Artwork & { originalIndex: number })[]>();
    
    // Group artworks by year
    sortedArtworks.forEach(artwork => {
      const year = normalizeDate(artwork.date).getFullYear();
      allYears.add(year);
      if (!artworksByYear.has(year)) {
        artworksByYear.set(year, []);
      }
      artworksByYear.get(year)!.push(artwork);
    });

    // Get year range
    const years = Array.from(allYears).sort((a, b) => 
      sorting === 'latest' ? b - a : a - b
    );
    ...
    const orderedYears = sorting === 'latest' ? 
      completeYears.sort((a, b) => b - a) : 
      completeYears.sort((a, b) => a - b);

    // Create timeline display array (with missing years)
    orderedYears.forEach((year, yearIndex) => {
      const artworksInYear = artworksByYear.get(year) || [];
      
      if (artworksInYear.length > 0) {
        // Add real artworks for this year
        artworksInYear.forEach(artwork => {
          const timeDifference = yearIndex === 0 ? 0 : MILLISECONDS_PER_YEAR;
          const timeMargin = yearIndex === 0 ? 0 : pixelsPerYear;
          
          const timelineArtwork = {
            ...artwork,
            timeDifference,
            timeMargin,
            yearsDifference: yearIndex === 0 ? 0 : 1,
            horizontalScrollPoint: 0,
            verticalScrollPoint: 0,
            isMissingYear: false
          };
          
          timelineDisplayArray.push(timelineArtwork);
        });
      } else {
        // Add missing year entry (only to display array)
        const timeDifference = yearIndex === 0 ? 0 : MILLISECONDS_PER_YEAR;
        const timeMargin = yearIndex === 0 ? 0 : pixelsPerYear;
        
        timelineDisplayArray.push({
          ...createMissingYearEntry(year),
          timeDifference,
          timeMargin,
          yearsDifference: yearIndex === 0 ? 0 : 1,
          isMissingYear: true
    ...
    // Create artwork scroll array (only real artworks, same order as display array)
    artworkScrollArray = timelineDisplayArray.filter(entry => !entry.isMissingYear);
  }

  // Step 4: Calculate scroll points for artwork array only
  let currentHorizontalPosition = desktopSideWidth;
  let currentVerticalPosition = 0;

  // Calculate scroll points for artwork scroll array (maps to timeline display positions)
  let displayIndex = 0;
  artworkScrollArray.forEach((artwork, artworkIndex) => {
    // Find the corresponding position in the timeline display array
    while (displayIndex < timelineDisplayArray.length && 
           timelineDisplayArray[displayIndex].isMissingYear) {
      // Skip missing years in display array to sync positions
      currentHorizontalPosition += artworkContainerWidth + timelineDisplayArray[displayIndex].timeMargin;
      currentVerticalPosition += artworkContainerHeight + timelineDisplayArray[displayIndex].timeMargin;
      displayIndex++;
    }

    // Calculate scroll points for current artwork
    ...
    currentHorizontalPosition += artworkContainerWidth;
    currentVerticalPosition += artworkContainerHeight;
    
    if (displayIndex < timelineDisplayArray.length - 1) {
      currentHorizontalPosition += timelineDisplayArray[displayIndex].timeMargin;
      currentVerticalPosition += timelineDisplayArray[displayIndex].timeMargin;
    }
    
    displayIndex++;
  });

  // Step 5: Calculate total dimensions based on timeline display array
  let totalHorizontalPosition = desktopSideWidth;
  let totalVerticalPosition = 0;

  timelineDisplayArray.forEach((entry, index) => {
    totalHorizontalPosition += artworkContainerWidth;
    totalVerticalPosition += artworkContainerHeight;
    
    if (index < timelineDisplayArray.length - 1) {
      totalHorizontalPosition += entry.timeMargin;
      totalVerticalPosition += entry.timeMargin;
    }
  });

  const totalTimelineWidth = totalHorizontalPosition + desktopSideWidth;
  const totalTimelineHeight = totalVerticalPosition;

  // Step 6: Calculate time span info
  let timeSpanInfo = null;
  
  if (artworkScrollArray.length > 0) {
    const dates = artworkScrollArray.map(artwork => normalizeDate(artwork.date));
